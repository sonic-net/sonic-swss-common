...
void ZmqServer::bind()
{
    ...
    if (!m_oneToOneSync) {
      // Increase recv buffer for use all bandwidth:
      // http://api.zeromq.org/4-2:zmq-setsockopt
      int high_watermark = MQ_WATERMARK;
      zmq_setsockopt(m_socket, ZMQ_RCVHWM, &high_watermark,
                     sizeof(high_watermark));
    }
...
void ZmqServer::mqPollThread()
{
    ...
    while (m_runThread)
    {
      ...
      if (rc == 0 || !(poll_item.revents & ZMQ_POLLIN)) {
        ...
        SWSS_LOG_DEBUG("zmq_poll timeout or invalid event rc: %d, revents: %d",
                       rc, poll_item.revents);
        continue;
      }
        ...
        if (m_oneToOneSync) {
          ...
          rc = zmq_recv(m_socket, m_buffer.data(), MQ_RESPONSE_MAX_COUNT,
                        ZMQ_DONTWAIT);
        }
        if (rc < 0)
        {
