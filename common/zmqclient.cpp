...
ZmqClient::ZmqClient(const std::string& endpoint) : ZmqClient(endpoint, "") {}
...
void ZmqClient::connect()
{
    ...
    if (!m_oneToOneSync) {
      // Increase send buffer for use all bandwidth:
      // http://api.zeromq.org/4-2:zmq-setsockopt
      int high_watermark = MQ_WATERMARK;
      zmq_setsockopt(m_socket, ZMQ_SNDHWM, &high_watermark,
    ...
    {
        {
            ...
            if (m_oneToOneSync) {
              ...
              rc = zmq_send(m_socket, m_sendbuffer.data(), serializedlen,
                            ZMQ_NOBLOCK);
            }
        }
        if (rc >= 0)
